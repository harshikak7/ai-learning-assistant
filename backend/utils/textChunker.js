...
//Returning empty array if pds has no readable content to prevent crashes
export const chunkText=(text, chunkSize=500,overlap=50)=>{
    ...
    //Try to upload by paragraphs (single or double newlines)
    const paragraphs = cleannedText.split(/\n+/).filter(p => p.trim().length >0);
    ...
    for( const paragraph of paragraphs){
        ...
        if (paragraphWordCount > chunkSize){
            ...
            //Split largee paragraph into word-based chunks
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
    ...
    // If query becomes empty after cleaning
